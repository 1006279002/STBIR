...
class CSTBIR_dataset():
    def __init__(self, data_path, image_files_path, batch_size, split, preprocess):
        ...
        # support CSV or JSON dataset files
        if str(data_path).lower().endswith('.json'):
            self.data = pd.read_json(data_path, lines=False)
        else:
            self.data = pd.read_csv(data_path)
        self.data = self.data[self.data['split'] == split]
        self.texts = self.data['text'].to_list()

        # category/label column (support legacy names)
        if 'object_category' in self.data.columns:
            self.categories = self.data['object_category'].to_list()
        elif 'label' in self.data.columns:
            self.categories = self.data['label'].to_list()
        else:
            # fallback: create empty categories
            self.categories = [None] * len(self.texts)

        # image filename column (support multiple names)
        if 'image_filename' in self.data.columns:
            image_list = self.data['image_filename'].to_list()
        elif 'image' in self.data.columns:
            image_list = self.data['image'].to_list()
        else:
            raise RuntimeError("Dataset must contain an 'image' or 'image_filename' column")

        self.images = [os.path.join(image_files_path, image_filename) for image_filename in image_list]

        # sketch filename column
        if 'sketch_filename' in self.data.columns:
            self.sketches = self.data['sketch_filename'].to_list()
        elif 'sketch' in self.data.columns:
            self.sketches = self.data['sketch'].to_list()
        else:
            raise RuntimeError("Dataset must contain a 'sketch' or 'sketch_filename' column")
